unit View.Nomenclature.Measures;
...
type
  TViewNomenclatureMeasures = class(TForm)
    DBGrid: TDBGrid;
    Panel: TPanel;
  ...
  private
    FObserver: IObserver;
    FViewModel: IViewModelNomenclatureMeasures;
  {$ENDREGION}
...
//    property Observer: IObserver read FObserver;
    property ViewModel: IViewModelNomenclatureMeasures read FViewModel;
  {$ENDREGION}
  ...
  procedure ShowViewNomenclatureMeasures;
...
uses
  ...
  ViewModel.Nomenclature.Measures,
  DataModule.Measures;

{$REGION 'Published Methods'}

procedure TViewNomenclatureMeasures.FormCreate(Sender: TObject);
begin
  FObserver := CreateObserverClass;
  FObserver.SetUpdateObserverMethod(ProcessNotification);

  FViewModel := CreateViewModelNomenclatureMeasures;
  FViewModel.Observable.Subscribe(FObserver);

  UpdateGUI;
end;

procedure TViewNomenclatureMeasures.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TViewNomenclatureMeasures.FormResize(Sender: TObject);
begin
  UpdateGUI;
end;

procedure TViewNomenclatureMeasures.SpeedButtonAttachClick(Sender: TObject);
begin
  ViewModel.Attach;
end;

procedure TViewNomenclatureMeasures.SpeedButtonModifyClick(Sender: TObject);
begin
  ViewModel.Modify;
end;

procedure TViewNomenclatureMeasures.SpeedButtonDetachClick(Sender: TObject);
begin
  ViewModel.Detach;
end;

procedure TViewNomenclatureMeasures.SpeedButtonExitClick(Sender: TObject);
begin
  Close;
end;
...
procedure TViewNomenclatureMeasures.ProcessNotification(const AModelNotification: IModelNotification);
begin
...
procedure TViewNomenclatureMeasures.UpdateGUI;
var
  LGUIRecord: TViewNomenclatureMeasuresGUIRecord;
begin
...
procedure ShowViewNomenclatureMeasures;
var
  LViewNomenclatureMeasures: TViewNomenclatureMeasures;
begin
  LViewNomenclatureMeasures := TViewNomenclatureMeasures.Create(Application.MainForm);
  LViewNomenclatureMeasures.Show;
end;
