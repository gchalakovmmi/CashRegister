...
//  Interfaces.ViewModel.Operation.Details,
  FireDAC.Stan.Intf,
  FireDAC.Stan.Option,
  FireDAC.Stan.Param,
  FireDAC.Stan.Error,
...
type
  TViewOperationDetails = class(TForm)
    ...
    FDMemTableDetailUniqueID: TStringField;
    FDMemTableTotalVAT: TStringField;
    FDMemTableDetailGID: TStringField;
  ...
  private
    FObserver: IObserver;
//    FViewModel: IViewModelOperationDetails;
  {$ENDREGION}
...
//    property ViewModel: IViewModelOperationDetails read FViewModel;
  {$ENDREGION}
  ...
  procedure ShowViewOperationDetails;
...
//  ViewModel.Operation.Details,
  DataModule.Actions;

{$REGION 'Published Methods'}

procedure TViewOperationDetails.FormCreate(Sender: TObject);
begin
  FObserver := CreateObserverClass;
  FObserver.SetUpdateObserverMethod(ProcessNotification);

//  FViewModel := CreateViewModelOperationDetails;
//  FViewModel.Observable.Subscribe(FObserver);

  UpdateGUI;
end;

procedure TViewOperationDetails.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TViewOperationDetails.FormResize(Sender: TObject);
begin
  UpdateGUI;
end;
...
procedure TViewOperationDetails.ProcessNotification(const AModelNotification: IModelNotification);
begin
...
procedure TViewOperationDetails.UpdateGUI;
//var
//  LGUIRecord: TViewOperationDetailsGUIRecord;
begin
...
procedure TViewOperationDetails.RefreshData;
var
  LList: TStringDynArray;
  I: Integer;
  LSale: IModelClassSale;
  LDetail: IModelClassSaleDetail;

  procedure AppendFromModel;
  begin
    FDMemTable.Append;
    FDMemTableDetailUniqueID.AsString := LSale.SaleUniqueID;
    FDMemTableTotalVAT.AsString := LDetail.TotalVAT;
    FDMemTableDetailGID.AsString := LDetail.GID;
    FDMemTableItemGID.AsString := LDetail.ItemGID;
    FDMemTableItemName.AsString := LDetail.ItemName;
    FDMemTableQuantity.AsString := LDetail.Quantity;
    FDMemTableClientBasePrice.AsString := LDetail.ClientPriceBase;
    FDMemTableDiscountValue.AsString := LDetail.DiscountValue;
    FDMemTableVATRate.AsString := LDetail.VATRate;
    FDMemTableTotal.AsString := LDetail.Total;
    FDMemTable.Post;
  end;
...
begin
  ...
  LList := TDirectory.GetFiles('.\продажби\2019\12\18\1', 'Sale*.json', TSearchOption.soTopDirectoryOnly);

  for I := 0 to Length(LList) - 1 do begin
    LSale := CreateFromFileModelClassSale(LList[i]);
    for LDetail in LSale.Details.List do  begin
      AppendFromModel;
    end;
  end;

  LList := TDirectory.GetFiles('.\продажби\2019\12\18\2', 'Sale*.json', TSearchOption.soTopDirectoryOnly);
...
procedure TViewOperationDetails.ApplyFilter;
begin
//  FDMemTable.First;
...
procedure TViewOperationDetails.SpeedButtonExitClick(Sender: TObject);
begin
  Close;
end;

procedure TViewOperationDetails.SpeedButtonExportClick(Sender: TObject);
begin
  ApplyFilter;
end;

procedure TViewOperationDetails.SpeedButtonFilterClick(Sender: TObject);
begin
  RefreshData;
  ApplyFilter;
end;
...
procedure ShowViewOperationDetails;
var
  LViewOperationDetails: TViewOperationDetails;
begin
  LViewOperationDetails := TViewOperationDetails.Create(Application.MainForm);
  LViewOperationDetails.Show;
end;
