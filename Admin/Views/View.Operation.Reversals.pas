...
//  Interfaces.ViewModel.Operation.Reversals,
  FireDAC.Stan.Intf,
  FireDAC.Stan.Option,
  FireDAC.Stan.Param,
  FireDAC.Stan.Error,
...
type
  TViewOperationReversals = class(TForm)
    ...
    FDMemTableReversalDate: TStringField;
    FDMemTableReversalTime: TStringField;
  ...
  private
    FObserver: IObserver;
//    FViewModel: IViewModelOperationReversals;
  {$ENDREGION}
...
//    property ViewModel: IViewModelOperationReversals read FViewModel;
  {$ENDREGION}
  ...
  procedure ShowViewOperationReversals;
...
//  ViewModel.Operation.Reversals,
  DataModule.Actions;

{$REGION 'Published Methods'}

procedure TViewOperationReversals.FormCreate(Sender: TObject);
begin
  FObserver := CreateObserverClass;
  FObserver.SetUpdateObserverMethod(ProcessNotification);

//  FViewModel := CreateViewModelOperationReversals;
//  FViewModel.Observable.Subscribe(FObserver);

  UpdateGUI;
end;

procedure TViewOperationReversals.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TViewOperationReversals.FormResize(Sender: TObject);
begin
  UpdateGUI;
end;
...
procedure TViewOperationReversals.ProcessNotification(const AModelNotification: IModelNotification);
begin
...
procedure TViewOperationReversals.UpdateGUI;
//var
//  LGUIRecord: TViewOperationReversalsGUIRecord;
begin
...
procedure TViewOperationReversals.SpeedButtonExitClick(Sender: TObject);
begin
  Close;
end;
...
procedure ShowViewOperationReversals;
var
  LViewOperationReversals: TViewOperationReversals;
begin
  LViewOperationReversals := TViewOperationReversals.Create(Application.MainForm);
  LViewOperationReversals.Show;
end;
