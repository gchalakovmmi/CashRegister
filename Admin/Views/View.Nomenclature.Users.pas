...
type
  TViewNomenclatureUsers = class(TForm)
    DBGrid: TDBGrid;
    Panel: TPanel;
  ...
  private
    FObserver: IObserver;
    FViewModel: IViewModelNomenclatureUsers;
  {$ENDREGION}
...
//    property Observer: IObserver read FObserver;
    property ViewModel: IViewModelNomenclatureUsers read FViewModel;
  {$ENDREGION}
  ...
  procedure ShowViewNomenclatureUsers;
...
uses
  ...
  ViewModel.Nomenclature.Users,
  DataModule.Users;

{$REGION 'Published Methods'}

procedure TViewNomenclatureUsers.FormCreate(Sender: TObject);
begin
  FObserver := CreateObserverClass;
  FObserver.SetUpdateObserverMethod(ProcessNotification);

  FViewModel := CreateViewModelNomenclatureUsers;
  FViewModel.Observable.Subscribe(FObserver);

  UpdateGUI;
end;

procedure TViewNomenclatureUsers.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TViewNomenclatureUsers.FormResize(Sender: TObject);
begin
  UpdateGUI;
end;

procedure TViewNomenclatureUsers.SpeedButtonAttachClick(Sender: TObject);
begin
  ViewModel.Attach;
end;

procedure TViewNomenclatureUsers.SpeedButtonModifyClick(Sender: TObject);
begin
  ViewModel.Modify;
end;

procedure TViewNomenclatureUsers.SpeedButtonDetachClick(Sender: TObject);
begin
  ViewModel.Detach;
end;

procedure TViewNomenclatureUsers.SpeedButtonExitClick(Sender: TObject);
begin
  Close;
end;
...
procedure TViewNomenclatureUsers.ProcessNotification(const AModelNotification: IModelNotification);
begin
...
procedure TViewNomenclatureUsers.UpdateGUI;
var
  LGUIRecord: TViewNomenclatureUsersGUIRecord;
begin
...
procedure ShowViewNomenclatureUsers;
var
  LViewNomenclatureUsers: TViewNomenclatureUsers;
begin
  LViewNomenclatureUsers := TViewNomenclatureUsers.Create(Application.MainForm);
  LViewNomenclatureUsers.Show;
end;
