...
begin
	...
	try
		try
			...
			case MyFiscDevice of
				dt_FP2000_KL:
					begin
						...
						if tmp_Int <> 0 then
						begin
							SHOW_MYALERT(FP2000_KL.GET_LASTERROR(0));
							Exit;
						end;
						if not TryStrToInt(C2, iC2) then Exit;
						iC2 := iC2 - 1;
						tmp_Int := FP2000_KL.CMD_119_0_1(iC2.ToString, F_RESULT, FDocs, DT, SK);
						if tmp_Int <> 0 then
						begin
							SHOW_MYALERT(FP2000_KL.GET_LASTERROR(0));
							Exit;
						...
						INPUT_STR := 'R,' + FDocs;
						OUTPUT_STR := '';
						if not FP2000_KL.CUSTOM_CMD(119, INPUT_STR, OUTPUT_STR) then
						begin
							SHOW_MYALERT(FP2000_KL.GET_LASTERROR(0));
							Exit;
						end
						else mem_LOG.Lines.Add(copy(OUTPUT_STR,3,length(OUTPUT_STR)));
						repeat
							tmp_Int := FP2000_KL.CMD_119_3_0(F_RESULT, THE_TEXT);
							if tmp_Int <> 0 then
							begin
								SHOW_MYALERT(FP2000_KL.GET_LASTERROR(0));
								Exit;
							end
							else mem_LOG.Lines.Add(THE_TEXT);
						until (F_RESULT = 'F');
					end
				else
					begin
						SHOW_MYALERT('This example is only for model "FP-2000 KL"');
						Exit
					end;
			end;
			...
			On E:Exception do SHOW_MYALERT(E.Message);
		end;
	finally
		fmMainForm.Update;
	end;
