...
//      LDataBase.Params.Add('path=.\');
      ...
      TFile.AppendAllText('log.txt', ASQL+#$0D, TEncoding.UTF8);
      ShowMessage('Проблем при работа с временните файлове!'+#$0D+ASQL+#$0D);
    end;
  finally
    LDataBase.Free;
  end;
...
function TModelClassSale.GetReceiptID: String;
begin
  Result := FReceiptID;
end;

procedure TModelClassSale.SetReceiptID(const AValue: String);
begin
  FReceiptID := AValue;
end;
...
begin
  ...
  ReceiptID := DataSet.FieldByName('ReceiptID').AsString;
...
begin
  ...
  DataSet.FieldByName('ReceiptID').AsString := ReceiptID;
...
begin
  ...
  Result.AddPair('ReceiptID', ReceiptID);
...
begin
  ...
  LCreatedDate := EncodeDate(
    2000+CreatedDate.Substring(6,2).ToInteger,
    CreatedDate.Substring(3,2).ToInteger,
    CreatedDate.Substring(0,2).ToInteger
  );
...
begin
  ...
  LSQL :=
    ...
    AReversal.PackCoeff+', '+
    // QUANTITY
    FormatFloat('0.000', _Round(-AReversal.Quantity.ToDouble , 0.001))+', '+
    // TOTALQUANTITY
...
begin
  ...
  TGeneratorGIDs.CommitGIDByName('SaleID');
  CreatedDate := FormatDateTime('dd-mm-yy', Date);
  CreatedTime := FormatDateTime('hh:mm:ss', Time);
  Stage := 'отворена';
  SaveToFile;
...
begin
  Due := FormatFloat('0.00', _Round(Due.ToDouble + ASaleDetail.Total.ToDouble, 0.01));
...
begin
  Due := FormatFloat('0.00', _Round(Due.ToDouble + ASaleCancellation.Total.ToDouble, 0.01));
  Returned := (CashPayment.ToDouble + VoucherPayment.ToDouble + CardPayment.ToDouble - Due.ToDouble).ToString;
...
procedure TModelClassSale.VoucherTotal;
begin
...
procedure TModelClassSale.CardTotal;
begin
...
begin
  ...
  ReceiptID := (AJSONObject.GetValue('ReceiptID') as TJSONString).Value;
