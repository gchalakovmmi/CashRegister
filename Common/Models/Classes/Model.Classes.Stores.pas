...
var
  LText: String;
begin
  if not TFile.Exists(TPath.Combine(G.StructureFolder, 'Stores.json')) then Exit(nil);

  LText := TFile.ReadAllText(TPath.Combine(G.StructureFolder, 'Stores.json'), TEncoding.UTF8);
  Result := CreateFromJSONModelClassStores(TJSONObject.ParseJSONValue(LText) as TJSONArray);
end;
...
begin
  TFile.WriteAllText(TPath.Combine(G.StructureFolder, 'Stores.json'), ToJSON.ToString, TEncoding.UTF8);
end;
