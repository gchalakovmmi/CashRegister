...
function TModelClassUser.GetPosition: String;
begin
  Result := FPosition;
end;

procedure TModelClassUser.SetPosition(const AValue: String);
begin
  FPosition := AValue;
end;
...
procedure TModelClassUser.AssignRole(const ARole: IModelClassRole);
begin
  if not FRoles.List.Contains(ARole) then begin
    FRoles.List.Add(ARole);
  end;
end;

function TModelClassUser.HaveRole(const ARole: IModelClassRole): Boolean;
begin
  Result := FRoles.List.Contains(ARole);
end;
...
begin
  for LRole in FRoles.List do begin
    if LRole.Name = ARoleName then Exit(True);
  end;
  Exit(False);
end;

procedure TModelClassUser.AssignPermission(const APermission: IModelClassPermission);
begin
  if not FPermissions.List.Contains(APermission) then begin
    FPermissions.List.Add(APermission);
  end;
end;

function TModelClassUser.Can(const APermission: IModelClassPermission): Boolean;
begin
  Result := FPermissions.List.Contains(APermission);
end;
...
begin
  for LPermission in FPermissions.List do begin
    if LPermission.Name = APermissionName then Exit(True);
  end;
  Exit(False);
end;
...
begin
  ...
  Position := DataSet.FieldByName('Position').Value;
...
begin
  ...
  DataSet.Edit;
  DataSet.FieldByName('Position').Value := Position;
...
begin
  ...
  FPermissions := CreateModelClassPermissions;
  FRoles := CreateModelClassRoles;
end;

constructor TModelClassUser.CreateFromJSON(const AJSONObject: TJSONObject);
begin
  inherited CreateFromJSON(AJSONObject);
  Position := (AJSONObject.GetValue('Position') as TJSONString).Value;
