...
begin
  ...
  while LDate <= AEndDate do begin
      ...
      for i := 0 to Length(LList) - 1 do begin
        ...
        if (
          ...
          for LJSONValue in LJSONArray do begin
            ...
            LDetailVATRate := (LJSONObject2.GetValue('VATRate') as TJSONString).Value;
            LDetailTotal := (LJSONObject2.GetValue('Total') as TJSONString).Value;

            LTotal := LTotal + LDetailTotal.ToExtended;
            if LDetailVATRate = '0.00' then begin
              LTotal0 := LTotal0 + (LJSONObject2.GetValue('Total') as TJSONString).Value.ToExtended;
            end;
            if LDetailVATRate = '9.00' then begin
              LTotal9 := LTotal9 + (LJSONObject2.GetValue('Total') as TJSONString).Value.ToExtended;
            end;
            if LDetailVATRate = '20.00' then begin
    ...
    LExportText := LExportText +
      ...
      FormatFloat('0.00', LTotal).PadLeft(15)+
      FormatFloat('0.00', LTotal0).PadLeft(15)+
      FormatFloat('0.00', LTotal9).PadLeft(15)+
      FormatFloat('0.00', LTotal20).PadLeft(15)+
      #$0D#$0A;
    LDate := LDate + 1;
  end;
